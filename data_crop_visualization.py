...
# VERSION = '0729_2_grid_projection_experiment'
...
def main():
    ...
    train_meta_dir = os.path.join(data_dir, f'metadata_{VERSION}_train.json')
    ...
    os.makedirs(f'{OUTPUT_PATH}/{VERSION}/cropped', exist_ok=True)
    os.makedirs(f'{OUTPUT_PATH}/{VERSION}/marked', exist_ok=True)
    ...
    content = []
    for meta_dir in [train_meta_dir, test_meta_dir]:
    ...
    for idx, _ in enumerate(tqdm(image_paths)):
        ...
        image.save(f'{OUTPUT_PATH}/{VERSION}/cropped/{time_stamp}_{gt}.png')
        ...
        cv2.imwrite(f'{OUTPUT_PATH}/{VERSION}/marked/{time_stamp}_{gt}_marked.png', image)
